...
from coquo.tenant import TenantError, TenantRegistry, validate_tenant_id, workspace_fingerprint
from coquo.tools.web_transport import PinnedWebGetTransport, WebGetTransport, WebTransportError
...
MARKETPLACE_LEDGER_SCHEMA_VERSION = 2
...
MARKETPLACE_INDEX_ENVELOPE_VERSION = 1
MAX_MARKETPLACE_INDEX_BYTES = 2 * 1024 * 1024
...
@dataclass(frozen=True)
class MarketplaceIndexEnvelope:
    """Publisher-authenticated envelope for one immutable marketplace index."""
    ...
    public_key_id: str
    generated_at: str
    signature_algorithm: str = SIGNATURE_ED25519

    def __post_init__(self) -> None:
        if not isinstance(self.index, MarketplaceIndex):
            raise MarketplaceError("marketplace signed index is invalid")
        if (
            ...
            raise MarketplaceError("marketplace index publisher is invalid")
        _validate_key_id(self.public_key_id)
        if not isinstance(self.signature, str) or not self.signature or len(self.signature) > 1024:
            raise MarketplaceError("marketplace index signature is invalid")
        if self.signature_algorithm != SIGNATURE_ED25519:
            raise MarketplaceError("marketplace index signature algorithm is invalid")
        if (
            not isinstance(self.generated_at, str)
            or not self.generated_at
            or len(self.generated_at) > 64
            or "\n" in self.generated_at
            or "\r" in self.generated_at
        ):
            raise MarketplaceError("marketplace index timestamp is invalid")
        ...
        return _canonical_json(
            {
                ...
                "index": {
                    ...
                    "entries": [_entry_mapping(entry) for entry in self.index.entries],
                },
                "public_key_id": self.public_key_id,
                "publisher": self.publisher,
                "signature_algorithm": self.signature_algorithm,
                "schema_version": MARKETPLACE_INDEX_ENVELOPE_VERSION,
            }
        )
    ...
    @classmethod
    def from_mapping(cls, value: object) -> "MarketplaceIndexEnvelope":
        if not isinstance(value, dict) or set(value) != {
            ...
            raise MarketplaceError("marketplace signed index fields are invalid")
        if value["schema_version"] != MARKETPLACE_INDEX_ENVELOPE_VERSION:
            raise MarketplaceError("unsupported marketplace signed index")
        try:
            index = MarketplaceIndex.from_mapping(value["index"])
            return cls(
                index=index,
            ...
            raise MarketplaceError("marketplace signed index is invalid") from error


class MarketplaceIndexPublisher:
    """Create a signed JSON index without publishing or executing packages."""
    ...
    def sign(
        ...
        *,
        ...
        if not isinstance(private_key, bytes):
            raise MarketplaceError("marketplace private key is invalid")
        timestamp = generated_at or _marketplace_now()
        provisional = MarketplaceIndexEnvelope(
            index=index,
            publisher=publisher,
        ...
        try:
            ...
            signature = base64.b64encode(
                Ed25519PrivateKey.from_private_bytes(private_key).sign(provisional.signed_payload())
            ).decode("ascii")
        except Exception as error:
            raise MarketplaceError("marketplace index signing failed") from error
        return MarketplaceIndexEnvelope(
            index=index,
            publisher=publisher,
            signature=signature,
            public_key_id=public_key_id,
    ...
    @staticmethod
    def write(envelope: MarketplaceIndexEnvelope, path: Path) -> None:
        encoded = _canonical_json(envelope.as_mapping()) + b"\n"
        if len(encoded) > MAX_MARKETPLACE_INDEX_BYTES:
            raise MarketplaceError("marketplace signed index exceeds size limit")
...
class MarketplaceIndexFetcher:
    """Fetch and verify one signed index using the bounded public GET transport."""

    def __init__(self, *, transport: WebGetTransport | None = None) -> None:
        self.transport = transport or PinnedWebGetTransport()

    def fetch(
        self,
        url: str,
        *,
        trust_store: MarketplaceTrustStore,
        timeout_seconds: int = 10,
    ) -> MarketplaceIndexEnvelope:
        parsed = urlsplit(url)
        if parsed.query or parsed.fragment:
            raise MarketplaceError("marketplace index URL must not contain query or fragment")
        if parsed.scheme != "https" or not parsed.hostname:
            raise MarketplaceError("marketplace index URL must use HTTPS")
        try:
            request = getattr(self.transport, "request", None)
            if not callable(request):
                raise MarketplaceError("marketplace index transport cannot guarantee no redirects")
            response = request(
                "GET",
                url,
                headers={},
                ...
                max_response_bytes=MAX_MARKETPLACE_INDEX_BYTES,
            )
        except (WebTransportError, ValueError) as error:
            raise MarketplaceError(f"marketplace index fetch failed: {error}") from error
        if response.status_code != 200:
            raise MarketplaceError(f"marketplace index HTTP status {response.status_code}")
        try:
            envelope = MarketplaceIndexEnvelope.from_mapping(
                json.loads(response.body.decode("utf-8"))
            )
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise MarketplaceError("marketplace signed index is not valid JSON") from error
        trusted = trust_store.resolve(envelope.public_key_id)
        if trusted is None:
            raise MarketplaceError("marketplace index signing key is not trusted")
        publisher, public_key = trusted
        if publisher != envelope.publisher:
            raise MarketplaceError("marketplace index signing publisher mismatch")
        try:
            signature = base64.b64decode(envelope.signature, validate=True)
            from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PublicKey

            Ed25519PublicKey.from_public_bytes(public_key).verify(
                signature, envelope.signed_payload()
            )
        except Exception as error:
...
class MarketplaceCatalog:
    ...
    def __init__(
        ...
        *,
        ...
        if (tenant_registry is None) != (tenant_id is None):
            raise MarketplaceError("marketplace tenant registry and tenant ID must be paired")
        if tenant_registry is not None and tenant_id is not None:
            try:
                tenant_registry.require_workspace(tenant_id, workspace_fingerprint(root))
    ...
    def quarantine(self, entry: MarketplaceEntry, package: Path) -> MarketplacePackage:
        ...
        if self.tenant_registry is not None and self.tenant_id is not None:
            policy = self.tenant_registry.resolve(self.tenant_id)
            if policy is None:
                raise MarketplaceError("marketplace tenant is not configured")
            active = sum(
                item.status
                in {
            ...
            if active >= policy.max_marketplace_packages:
                raise MarketplaceError("tenant marketplace package quota is exhausted")
        scope_root = self.root if self.tenant_id is None else self.root / "tenants" / self.tenant_id
        destination = scope_root / "quarantine" / entry.name / entry.version
        destination.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
        ...
        info = MarketplacePackage(
            entry, destination, MarketplaceStatus.QUARANTINED, tenant_id=self.tenant_id
        )
        self._packages[self._key(entry.name, entry.version)] = info
    ...
    def approve(self, name: str, version: str) -> MarketplacePackage:
        ...
        info = MarketplacePackage(
            info.entry, info.root, MarketplaceStatus.APPROVED, tenant_id=info.tenant_id
        )
    ...
    def install(self, name: str, version: str) -> MarketplacePackage:
        ...
        scope_root = self.root if self.tenant_id is None else self.root / "tenants" / self.tenant_id
        destination = scope_root / "installed" / name / version
        destination.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
        if destination.exists():
            raise MarketplaceError("package version is already installed")
        shutil.copytree(info.root, destination, symlinks=False)
        info = MarketplacePackage(
            info.entry, destination, MarketplaceStatus.INSTALLED, tenant_id=info.tenant_id
        )
        self._packages[key] = info
    ...
    def rollback(self, name: str, version: str) -> MarketplacePackage:
        ...
        info = MarketplacePackage(
            info.entry, info.root, MarketplaceStatus.ROLLED_BACK, tenant_id=info.tenant_id
        )
        self._packages[key] = info
    ...
    def revoke(self, name: str, version: str) -> MarketplacePackage:
        ...
        info = MarketplacePackage(
            info.entry, info.root, MarketplaceStatus.REVOKED, tenant_id=info.tenant_id
        )
        self._packages[key] = info
    ...
    def _key(self, name: str, version: str) -> str:
        return f"{self.tenant_id or '-'}:{name}@{version}"
        ...
        try:
            ...
            for line in events:
                ...
                if not isinstance(value, dict) or set(value) != {
                    ...
                    raise ValueError
                if value["schema_version"] not in {1, MARKETPLACE_LEDGER_SCHEMA_VERSION}:
                    raise ValueError
                package = value["package"]
                expected_fields = {"entry", "root", "status", "previous_version"}
