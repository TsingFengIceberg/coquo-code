...
from leonervis_code.agent.tool_events import AgentPromptEvent
...
from leonervis_code.tools._workspace_paths import open_parent_directory
from leonervis_code.tools.command_sandbox import LinuxBubblewrapCommandSandbox
from leonervis_code.tools.run_command import RunCommandExecutionResult, RunCommandTool
...
_HIDDEN_TEST_DIRECTORY = ".leonervis-eval-tests"
...
def score_coding_task(
    ...
    *,
    ...
    hidden_fact = "not-run"
    try:
        with TemporaryDirectory(prefix="leonervis-task-score-") as temporary:
