...
To represent grep's two fields, in-memory `ToolUse` gained immutable canonical-JSON `ToolArguments` v1. At Foundation 1D, new `turn_committed` records used record-local schema v2 with `arguments_version + arguments`; legacy schema-v1 read/glob items converted to the same generic in-memory representation during replay without rewriting old JSONL, and resume appended only v2 at that stage. Assistant-tool-text persistence later advanced the writer to v3, and current multi-tool batching advances it to v4 while retaining v1/v2/v3 readers. Other Session records remain v1 and `context_compacted` remains compatible with v2/v3. Current Effective Context uses ctx-v3/v4 while replaying legacy ctx-v1/v2 checkpoints.
