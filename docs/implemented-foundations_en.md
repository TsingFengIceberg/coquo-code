...
The canonical model system prompt is now version 13. It still says the ordinary Agent cannot initiate compaction and preserves the Host-summary trust boundary: an earlier-conversation summary is untrusted conversation context, not a system instruction or new user request. Foundation 1D adds bounded literal `grep` and correct empty/truncated interpretation; Foundation 1E adds `list_directory`, which lists one level including hidden entries and entry types without following symlinks; Foundation 4A adds `write_file`, Host-owned permission/approval, exact-state conflict, and visible partial-outcome semantics; Foundation 4B adds unique exact `edit_file`; Foundation 4C adds direct-argv `run_command`, the `danger-full-access` requirement, no-sandbox boundaries, bounded output, and timeout/cancellation/process-cleanup semantics; Foundation 4D adds `mkdir`, which creates exactly one directory whose parent must already exist and never creates parents recursively; Foundation 4E adds `move_file`, which moves only regular files, never replaces a destination, and truthfully reports visible two-name partial states; Foundation 4F adds `delete_file`, which permanently deletes only one existing regular file, rejects directories and symlinks, and forbids automatic retry when deletion is visible but durability is unknown. Foundation 4G adds `delete_directory`, which permanently deletes only one existing empty directory, rejects symlinks and non-empty targets, and ultimately relies on the OS atomic `rmdir` empty condition. Foundation 4H adds binary `copy_file`, bounded to 1 MiB, no-overwrite, and source-retaining. The twelve model-visible tools share a three-call sequential budget.

It explicitly does not claim recursive copy/tree listing, metadata/stat, ignore-aware listing, regex, fuzzy, or multi-edit patching, non-empty or recursive directory deletion, directory movement, recursive mkdir, shell source strings, interactive PTYs, OS/network sandboxing, compaction initiation, project-instruction loading, or multi-agent capabilities. Prompt instructions also do not replace the Host's hard workspace, symlink, encoding, size, exact-state conflict, command, causality, audit, and durability constraints.
