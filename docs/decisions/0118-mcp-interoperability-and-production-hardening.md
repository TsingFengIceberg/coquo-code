...
Permit one SSE `data:` line to carry one complete bounded MCP message instead of imposing an unrelated 64 KiB transport-line ceiling. The decoded JSON-RPC message and complete HTTP response remain independently limited to 1 MiB, and event-count, JSON-depth, JSON-node, tool-count, and pagination limits remain unchanged. This supports servers such as the official GitHub MCP endpoint whose bounded `tools/list` response is serialized as one large SSE data line.

Recognize `x-mcp-header` only on direct string properties of the root tool input object. Its bounded ASCII name remains in the raw schema fingerprint but is removed from Provider projection. After live tool and schema revalidation, a remote `tools/call` may project a present bounded visible-ASCII string argument to the corresponding `MCP-Param-*` request header. Nested, duplicate, malformed, non-string, excessive, or control-bearing mappings fail before network dispatch, and projected names or values are not persisted in Session or Action Audit. This implements the narrow routing hint used by the official GitHub MCP server without accepting arbitrary schema extensions or model-selected HTTP headers.
