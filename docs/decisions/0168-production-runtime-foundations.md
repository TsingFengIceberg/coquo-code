...
Accepted as a bounded local-production hardening slice, including the
authenticated control-plane extensions described below.
...
- `RemoteWorkerFleet` and `coquo-remote-fleet` provide a bounded authenticated
  scheduler/control plane. Workers are explicitly registered, capability and
  permission ceilings are enforced, leases are durable, and revoked or expired
  work ends as `unknown`. The implementation is not a public internet fleet
  service and does not claim exactly-once effects.
- `TenantRegistry` binds configured tenant IDs to one workspace fingerprint and
  stores only token digests. A2A task routes, Fleet task submission/listing, and
  Marketplace package lifecycle can require the same tenant token; package
  records and quota accounting are tenant-partitioned.
- `MarketplaceIndexPublisher` and `MarketplaceIndexFetcher` provide a signed,
  HTTPS-only index exchange with Ed25519 verification and redirect rejection.
  They do not host a public marketplace or execute downloaded packages.
- `PrivilegedEvolutionBridge` stages protected self-evolution candidates and
  requires a Host approval receipt plus Host-owned apply/rollback callables.
  Model-produced content cannot directly alter the system prompt, permissions,
  sandbox, ToolSet, or AgentLoop.

## Security and recovery boundaries

These facilities remain local single-user or explicitly operator-exposed
control planes. They do not claim a hostile OS sandbox, exactly-once external
effects, a public internet worker fleet, public marketplace hosting, or a
multi-tenant cloud service. Tenant isolation is workspace-bound and token
digest-based; it is not a hosted identity, billing, or account system.
Credentials and provider/browser content are not written to the new ledgers.
Unknown outcomes, stale cursors, revoked keys, missing browser binaries, and
failed cleanup remain visible and fail closed.
