...
def test_tab_completion_returns_existing_slash_commands() -> None:
    ...
    assert complete_command("/", 11) == "/sandbox"
    assert complete_command("/", 12) == "/context"
    assert complete_command("/", 13) == "/usage"
    assert complete_command("/", 14) == "/output"
    assert complete_command("/", 15) == "/compact"
    assert complete_command("/", 16) == "/compactions"
    assert complete_command("/", 17) == "/provider"
    assert complete_command("/", 18) == "/model"
    assert complete_command("/", 19) == "/session"
    assert complete_command("/", 20) == "/resume"
    assert complete_command("/", 21) == "/clear"
    assert complete_command("/", 22) is None
