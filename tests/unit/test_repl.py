...
def test_tab_completion_returns_existing_slash_commands() -> None:
    ...
    assert complete_command("/", 17) == "/effort"
    assert complete_command("/", 18) == "/compact"
    assert complete_command("/", 19) == "/compactions"
    assert complete_command("/", 20) == "/provider"
    assert complete_command("/", 21) == "/search"
    assert complete_command("/", 22) == "/mcp"
    assert complete_command("/", 23) == "/skills"
    assert complete_command("/", 24) == "/hooks"
    assert complete_command("/", 25) == "/model"
    assert complete_command("/", 26) == "/session"
    assert complete_command("/", 27) == "/task"
    assert complete_command("/", 28) == "/child"
    assert complete_command("/", 29) == "/team"
    assert complete_command("/", 30) == "/workflow"
    assert complete_command("/", 31) == "/resume"
    assert complete_command("/", 32) == "/clear"
    assert complete_command("/", 33) is None
