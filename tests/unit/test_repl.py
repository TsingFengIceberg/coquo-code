...
def test_tab_completion_returns_existing_slash_commands() -> None:
    ...
    assert complete_command("/", 23) == "/model"
    assert complete_command("/", 24) == "/session"
    assert complete_command("/", 25) == "/task"
    assert complete_command("/", 26) == "/resume"
    assert complete_command("/", 27) == "/clear"
    assert complete_command("/", 28) is None
