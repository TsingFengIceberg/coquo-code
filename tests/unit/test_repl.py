...
def test_tab_completion_returns_existing_slash_commands() -> None:
    ...
    assert complete_command("/", 20) == "/search"
    assert complete_command("/", 21) == "/mcp"
    assert complete_command("/", 22) == "/skills"
    assert complete_command("/", 23) == "/hooks"
    assert complete_command("/", 24) == "/model"
    assert complete_command("/", 25) == "/session"
    assert complete_command("/", 26) == "/task"
    assert complete_command("/", 27) == "/child"
    assert complete_command("/", 28) == "/team"
    assert complete_command("/", 29) == "/resume"
    assert complete_command("/", 30) == "/clear"
    assert complete_command("/", 31) is None
