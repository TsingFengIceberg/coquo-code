...
def test_group_help_and_targeted_usage(tmp_path) -> None:
    ...
    assert dispatch_slash("/help unknown", session).message == (
        "Usage: /help [session|task|child|team|workflow|tools|git|context|provider|search|mcp|skills|hooks|policy|input]"
    )
    ...
    assert "Context ID: ctx-v29-" in context.message
