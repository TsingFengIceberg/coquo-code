...
> **Current status:** named provider profiles, Provider-native search, real/offline runtimes, resumable Sessions, workspace-root `AGENTS.md` instructions, both Eval layers, foreground durable multi-Stage Tasks, 22 ordinary bounded tools including Host-selected Brave or Tavily independent web search, four Stage-scoped coordination tools, one Task-admission tool, and three natural-language Task lifecycle tools are implemented. A Session enables Provider-native search by default only when the current Provider declares that capability; Brave and Tavily always start disabled. Every Stage remains an ordinary Turn, while file and Host-search actions continue through PermissionGate, approval, and Action Audit. Provider-native search is part of model generation and is not presented as a Host ToolUse. On Linux, `run_command` requires bubblewrap and seccomp isolation, with the workspace as the only Host-persistent writable area and command sockets denied. The ordinary Turn budget remains eight calls per response, 32 tool requests per user turn, and 24 provider invocations with a final text-only invocation.
...
Search has two distinct boundaries. Provider-native search is projected into model generation by the Profile's versioned adapter. A Session enables it by default when the current Provider declares availability. It is not a Leonervis ToolUse, PermissionGate action, or Action Audit entry; search execution and billing are Provider-owned. OpenAI and DeepSeek V4 Flash use Responses, so Provider `web_search_call` items persist as Provider-owned Session history and return unchanged in later requests; other native-search dialects retain their adapters. A completed Responses invocation may contain an individual failed search action. Safe citations normalize into a `Sources:` section; unsafe or incompatible citations are discarded with a redacted warning instead of failing the completed Turn. `/search mode auto|required` controls optional versus forced Provider search, while `/search domains` and `/search context` set adapter-supported process-local restrictions. Unsupported combinations reject explicitly.

Independent `web_search(query, max_results)` calls fixed Brave or Tavily APIs through the Host and always starts disabled, even when credentials exist. `/search use brave|tavily` makes that source primary. `/search use provider tavily` instead keeps Provider search primary and exposes Tavily-backed Host search to the model only as a fallback after the model observes a failed Provider search or unavailable structured citations. The Host does not infer a query, automatically invoke the fallback, fan out, or fuse results. `/search status|sources` inspects state, and `/search reset` restores “Provider when available, otherwise none.” Host search remains a `network-read` action allowed only by `danger-full-access` under `ask/auto`, with the existing query, response, timeout, ActionIdentity, and partial-result bounds. The terminal shows content-free Provider search phases and degradation summaries; Session previews derive only call count, failure count, action types, source count, and citation count. Credentials come only from environment variables and never enter Session or audit.
...
| `/tools catalog [tool-name]` | Show permission and Prompt/Stage availability for all 30 canonical tools, or one tool's argument schema and major hard boundaries |
...
The ordinary-prompt model-visible order is `read_file, glob, grep, write_file, edit_file, run_command, mkdir, move_file, delete_file, delete_directory, list_directory, copy_file, read_file_lines, stat_path, list_tree, grep_regex, patch_file, git_status, git_diff, git_log, git_show, web_search, task_propose_start, task_accept_admission, task_accept_plan, task_confirm_completion`. After the 22 ordinary tools, the complete catalog contains four Stage proposal tools, `task_propose_start`, and three ordinary-Prompt lifecycle tools, for 30 definitions total; each Task Stage exposes only its matching least-capability subset. One provider response may contain up to eight ordered tool calls; one user turn admits at most 32 tool requests and 24 provider invocations, with the final invocation restricted to text. The Host parses and budget-checks the complete batch before sequential execution. A non-successful ordinary action explicitly skips later calls in that batch, and a batch that cannot fit the remaining budget gets zero execution. Ordinary file, command, and search tools still pass through permission, approval, execution, and Action Audit; Task coordination calls are response-exclusive, share the budget, and receive no Action lease.
...
Provider batching, structured tool ledgers, streaming/Markdown terminal output, Session management and usage audit, read-only Git observation, the fail-closed command sandbox, Foundation 5A, both Eval layers, structured model Task coordination, natural-language Task lifecycle, Brave/Tavily independent search, Provider-native search, and OpenAI Responses are complete. Current versions are canonical system prompt v32, provider adapter contract v35, Profile schema v5/fingerprint v4, ToolArguments v1, ActionIdentity v1, `turn_committed` schema v9, `turn_failed` schema v2, new `session_resumed` records at v2, Action Audit schema v1, new `context_compacted` records at v4, and the existing Task schemas; current Effective Context representations are `ctx-v7`/`ctx-v8`. Existing Session and Task transcripts are not rewritten, and the current empty full-context identity without project instructions is `ctx-v7-3ac4ba4e6ffa39c1184cfff6cc4200eb30607553fdf886451c0d967765ff0432`. Responses native search supports semantic search-phase streaming, failed-action preservation, citation degradation, and redacted Session summaries, while other native-search dialects retain reliable buffered invocation. Multi-source fan-out/fusion, general `web_fetch`, MCP, background Tasks, scheduling, SubAgents, teams, worktree orchestration, parallel Stages/Actions, general automatic retry, resource quotas, Host sandbox bypass, real-model Eval leaderboards, and remote services remain unavailable. See [ADR 0104](./docs/decisions/0104-openai-responses-protocol-and-provider-owned-history.md) for Responses and [ADR 0105](./docs/decisions/0105-provider-search-resilience-controls-and-observability.md) for Provider search resilience and controls.
