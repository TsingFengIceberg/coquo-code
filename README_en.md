...
Provider batching, redacted live activity, mixed responses, streaming, and TTY Markdown rendering are complete; Foundation 5A remains deferred. Current versions are canonical system prompt v18, provider adapter contract v20, ToolArguments v1, ActionIdentity v1, `turn_committed` schema v4, Action Audit schema v1, `context_compacted` v2/v3 replay, and current `ctx-v3`/`ctx-v4` representations. Legacy Session v1/v2/v3 and `ctx-v1`/`ctx-v2` checkpoints remain compatible; the empty full-context identity is `ctx-v3-9007cd576ff595afb6a103a199437d28580836f2a3a5b551819f0f8574d4cf80`. Recursive copying/deletion, ignore-aware or indexed search, fuzzy/free-form patching, directory movement, non-empty deletion, recursive mkdir, shell source strings, interactive PTYs, network tools, automatic retry/fallback, parallel tools, multiple agents, and remote services remain unavailable. See [ADR 0055](./docs/decisions/0055-bounded-multi-tool-response-batches.md) for the current batch design.
