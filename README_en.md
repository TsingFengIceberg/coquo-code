...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, and 17 bounded sequential tools are implemented: the previous read/write/command/directory/file operations plus `read_file_lines`, `stat_path`, `list_tree`, `grep_regex`, and `patch_file`. The additions provide line-addressed reads, no-follow metadata, bounded recursive trees, process-isolated regex search, and structured multi-edit patching. All retain the shared three-call budget, PermissionGate, Action Audit, Session, and Effective Context boundaries. Foundation 5A has not started; shell source strings, interactive PTYs, OS sandboxing, fuzzy/free-form patches, recursive directory copying/deletion, non-empty directory deletion, and directory movement remain unavailable.
...
The fixed model-visible order is `read_file, glob, grep, write_file, edit_file, run_command, mkdir, move_file, delete_file, delete_directory, list_directory, copy_file, read_file_lines, stat_path, list_tree, grep_regex, patch_file`; all 17 share at most three sequential calls per user turn. The four added read/navigation tools reuse `workspace-read`, `patch_file` reuses `workspace-overwrite`, and existing action classes remain unchanged. Every tool passes through the shared PermissionGate and durable Action Audit.

Tool batches A/B/C are complete, while Foundation 5A remains deferred. The canonical system prompt is v14, the provider adapter contract is v15, and the empty full-context identity is `ctx-v1-ac2b833bb46894c250e2b31370d47911b3464cfa2c71c23ded504f0ea65fd4cf`. ToolArguments v1, ActionIdentity v1, `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. Recursive copying/deletion, ignore-aware or indexed search, fuzzy/free-form patching, directory movement, non-empty deletion, recursive mkdir, shell source strings, interactive PTYs, network tools, automatic retry/fallback, parallel tools, multiple agents, and remote services remain unavailable. See [ADRs 0037](./docs/decisions/0037-batch-a-bounded-workspace-navigation.md), [0038](./docs/decisions/0038-batch-b-process-isolated-regex-grep.md), and [0039](./docs/decisions/0039-batch-c-structured-exact-multi-edit-patch.md).
