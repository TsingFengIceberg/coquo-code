...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, a bounded sequential `read_file`/`glob`/literal `grep`/`write_file`/`edit_file`/`run_command`/`mkdir`/`move_file`/`delete_file`/`delete_directory` tool loop, provider-owned model limits, target-specific preflight, switch-time screening, provider-neutral Effective Context, manual and automatic compaction, and target-aware resume are implemented. Foundation 4A connects permission, approval, durable Action Audit, and controlled writes end to end; Foundation 4B adds unique exact edit; Foundation 4C adds controlled local command execution, Foundation 4D adds controlled single-directory creation, and Foundation 4E adds controlled no-overwrite regular-file movement, Foundation 4F adds controlled regular-file deletion, and Foundation 4G now adds controlled empty-directory deletion. Shell source strings, interactive PTYs, OS sandboxing, regex/fuzzy patching, recursive or non-empty directory deletion, and directory movement remain unavailable.
...
The current model-visible surface has the fixed order `read_file`, `glob`, literal `grep`, complete-content `write_file`, unique exact `edit_file`, direct-argv `run_command`, single-directory `mkdir`, regular-file `move_file`, regular-file `delete_file`, and empty-directory `delete_directory`; all ten share at most three sequential calls per user turn. Commands still require `danger-full-access`. Mkdir, move_file, and delete_file/delete_directory use `workspace-create`, `workspace-move`, and `workspace-delete` respectively and share PermissionGate, approval, and durable Action Audit with other side-effecting tools.

Foundation 4G is complete. The canonical system prompt is v11, the provider adapter contract is v12, and the empty full-context identity is `ctx-v1-64ce77996397ddd1f84a27248ddd3e47224948563db506e3bfbda96939799406`; ToolArguments v1, ActionIdentity v1, `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. Regex/index/ignore-aware search, fuzzy or multi-edit patching, directory movement, non-empty or recursive deletion, recursive mkdir, shell source strings, interactive PTYs, network tools, streaming, automatic retry/fallback, parallel tools, multi-agent operation, and remote services remain unavailable. Any future recursive-deletion work must separately design tree bounds, symlink handling, partial-tree effects, and recovery rather than widening the current `delete_directory` contract. See [ADR 0034](./docs/decisions/0034-foundation-4g-controlled-empty-directory-deletion.md).
