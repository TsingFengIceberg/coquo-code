...
Submitted user messages consistently start with `› ` and assistant bodies with `• `. Explicit newlines and terminal wrapping continue from the body column after the marker, while an indented low-intensity short rule separates conversation message blocks. Host information such as tool, context, usage, and slash results is indented and uses reduced-intensity styling when color is available, while warnings, approvals, and errors remain prominent. Terminal protocols cannot reliably select a smaller font for individual lines; `NO_COLOR=1` removes color and dim styling while preserving roles, separators, indentation, and layout.
