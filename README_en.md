...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, a bounded sequential `read_file`/`glob`/literal `grep`/`write_file`/`edit_file`/`run_command`/`mkdir`/`move_file`/`delete_file`/`delete_directory`/`list_directory` tool loop, provider-owned model limits, target-specific preflight, switch-time screening, provider-neutral Effective Context, manual and automatic compaction, and target-aware resume are implemented. Foundations 4A–4G connect controlled writes, exact edit, commands, single-directory creation, regular-file movement, and file/empty-directory deletion; Foundation 1E now adds bounded one-level directory inspection. Shell source strings, interactive PTYs, OS sandboxing, regex/fuzzy patching, recursive directory listing or deletion, non-empty directory deletion, and directory movement remain unavailable.
...
The current model-visible surface has the fixed order `read_file`, `glob`, literal `grep`, complete-content `write_file`, unique exact `edit_file`, direct-argv `run_command`, single-directory `mkdir`, regular-file `move_file`, regular-file `delete_file`, empty-directory `delete_directory`, and one-level `list_directory`; all eleven share at most three sequential calls per user turn. Commands still require `danger-full-access`. List_directory reuses `workspace-read`; mkdir, move_file, and delete_file/delete_directory use `workspace-create`, `workspace-move`, and `workspace-delete` respectively. Every tool passes through the shared PermissionGate and durable Action Audit.

Foundation 1E is complete. The canonical system prompt is v12, the provider adapter contract is v13, and the empty full-context identity is `ctx-v1-7776df09d6ace66621cee46719755307b7d816bccde25f61064b4205c689b3b2`; ToolArguments v1, ActionIdentity v1, `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. Recursive tree listing, metadata/stat, ignore-aware listing, regex/index search, fuzzy or multi-edit patching, directory movement, non-empty or recursive deletion, recursive mkdir, shell source strings, interactive PTYs, network tools, streaming, automatic retry/fallback, parallel tools, multi-agent operation, and remote services remain unavailable. See [ADR 0035](./docs/decisions/0035-foundation-1e-bounded-directory-listing.md).
