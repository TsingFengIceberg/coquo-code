...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, workspace-root `AGENTS.md` instructions, both Eval layers, foreground durable multi-Stage Tasks, and 21 bounded tools are implemented. Tasks now support structured acceptance, feedback-driven no-tools Reflection, ordinary-boundary Correction, provenance-linked plan revision, a bounded foreground Driver, Task context checkpoints, and human pause/resume/next controls; every Stage still reuses the ordinary Turn, PermissionGate, approval, Action Audit, and Session commit path. On Linux, `run_command` requires bubblewrap and seccomp isolation: the workspace is the only Host-persistent writable area and network sockets are denied; an unavailable sandbox never degrades to direct Host execution. The ordinary Turn budget remains eight calls per response, 32 tool requests per user turn, and 24 provider invocations with a final text-only invocation.
...
`/task` remains the human/operator interface; models do not manage Tasks by generating slash commands. The lower layer now supports an exact per-Turn tool subset and a commit-coupled Task proposal channel: future model coordination calls remain separate from file Actions, reach the Task state machine only after the Session Turn commits, and can be recovered from committed ToolUse plus the Host ledger. No concrete Task coordination tool is public yet, so current model behavior is unchanged.
...
Provider batching, structured tool ledgers, streaming/Markdown terminal output, Session management and usage audit, read-only Git observation, the fail-closed command sandbox, Foundation 5A, both Eval layers, and adaptive foreground durable Tasks are complete. Current versions are canonical system prompt v26, provider adapter contract v27, ToolArguments v1, ActionIdentity v1, `turn_committed` schema v8, `turn_failed` schema v2, new `session_resumed` records at v2, Action Audit schema v1, new `context_compacted` records at v4, new Task Stage records at v2, new Task plan proposals at v2, and new Task acceptance, reflection, pause, and Task-checkpoint records at v1; current Effective Context representations remain `ctx-v5`/`ctx-v6`. Legacy `session_resumed` v1, header-only Tasks, Task Stage v1, Task plan proposal v1, and historical context identities/checkpoints remain compatible; the current empty full-context identity without project instructions is `ctx-v5-4f33f80622dd368a51b4046c5292951f2dd42fdb05b3d9be798dfa6b5f2457a4`. Background Tasks, scheduling, SubAgents, teams, worktree orchestration, parallel Stages/Actions, automatic reviewer spending, general automatic retry, network tools, resource quotas, Host sandbox bypass, real-model Eval leaderboards, and remote services remain unavailable. See [ADR 0094](./docs/decisions/0094-task-proposal-control-boundary.md) for the Task proposal foundation and [ADR 0092](./docs/decisions/0092-adaptive-foreground-task-orchestration.md) for adaptive Task orchestration.
