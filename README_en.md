...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, and 17 bounded sequential tools are implemented: the previous read/write/command/directory/file operations plus `read_file_lines`, `stat_path`, `list_tree`, `grep_regex`, and `patch_file`. The additions provide line-addressed reads, no-follow metadata, bounded recursive trees, process-isolated regex search, and structured multi-edit patching. All retain the shared six-call budget, PermissionGate, Action Audit, Session, and Effective Context boundaries. Foundation 5A has not started; shell source strings, interactive PTYs, OS sandboxing, fuzzy/free-form patches, recursive directory copying/deletion, non-empty directory deletion, and directory movement remain unavailable.
...
`move_file(source, destination)` is the eighth model-visible tool and moves one existing regular file to one missing workspace-relative destination. It uses the dedicated `workspace-move` class: `read-only` denies it, while both writable modes follow ask/auto policy. Both parents must exist, paths may not contain symlinks, source and destination must share a filesystem, and no existing destination entry is ever replaced. The Host revalidates the source, destination, and both parents after approval, then performs exclusive hard-link creation, destination-parent fsync, source unlink, and source-parent fsync. Because this is not a one-step transaction, a visible destination with a retained source or uncertain durability returns a partial result that requires inspection of both paths and must not be retried automatically.

`delete_file(path)` is the ninth model-visible tool and permanently deletes one existing non-symlink regular file. It uses the dedicated `workspace-delete` class: `read-only` denies it, while both writable modes follow ask/auto policy. The parent must already exist and no path component may be a symlink; a missing target, directory, or symlink is hard-rejected before permission evaluation and creates no Action Audit. After approval the Host revalidates target and parent identity, then unlinks the name and fsyncs the parent. If the name is gone but parent durability cannot be confirmed, the result is partial and must not be retried automatically; no trash, backup, or undo is provided.

`delete_directory(path)` is the tenth model-visible tool and permanently deletes one existing empty non-symlink directory under the same `workspace-delete` class. A non-empty target, regular file, symlink, missing target, or unsafe parent is hard-rejected before permission evaluation. After approval the Host revalidates target, parent, and empty state; it durably records `action_execution_started`, relies on the OS `rmdir` atomic empty condition, and fsyncs the parent. A removed name with unknown durability is partial and must not be retried automatically; recursive deletion, trash, backup, and undo are unavailable.

`list_directory(path)` is the eleventh model-visible tool and read-only lists one directory's direct children; `.` denotes the workspace root. Results are sorted by UTF-8 path and returned as JSONL containing `path` plus a `file`, `directory`, `symlink`, or `other` type. Hidden entries are included, but the tool does not recurse, follow symlinks, or read content. It scans at most 10,000 direct entries, returns at most 200 records, and emits at most 32 KiB; the scan limit fails safely, while count or byte limits append `{"truncated":true}`. It is a `workspace-read`, requires no human approval in any permission mode, and still passes through Action Audit and the shared tool budget.

`copy_file(source, destination)` is the twelfth model-visible tool and copies one regular file of at most 1 MiB to a missing destination while retaining the source. Both parents must already exist, paths may contain no symlink, and no existing destination entry is ever replaced. It copies raw bytes and only basic `rwx` permission bits. `read-only` denies it, while both writable modes follow `workspace-create` ask/auto policy. After approval the Host revalidates source content, both parents, and destination absence, then installs an fsynced temporary file exclusively. A visible destination with cleanup or durability uncertainty returns a partial result and must not be retried automatically.

`read_file_lines(path, start_line, line_count)` reads a 1-based logical-line range from one UTF-8 regular file of at most 1 MiB, with at most 200 lines and 32 KiB of output; it is intended for sections after a truncated `read_file`. `stat_path(path)` reports no-follow file/directory/symlink/other type, basic `rwx` mode, and mtime, plus size for regular files. `list_tree(path, max_depth)` recursively lists a bounded tree through at most 16 levels, includes hidden entries, and never follows symlinks; scan caps fail the whole call while the 500-result or 32-KiB cap returns a truncation sentinel. All three are `workspace-read` actions.

`grep_regex(pattern, include)` applies a case-sensitive Python regex independently to each logical line while retaining literal grep's candidate-file, UTF-8, read-volume, match-count, and 32-KiB output bounds. Selection, reads, and matching run in a separate worker process, and the Host waits at most one second before terminating and reaping it. It has no cross-line mode, flags argument, index, or ignore-aware behavior; literal `grep` remains preferred when a literal is sufficient.

`patch_file(path, edits)` atomically applies 1–16 structured exact replacements to one existing UTF-8 regular file. Every `old_text` must occur exactly once in the same original snapshot and matches may not overlap; the complete candidate is built only after every edit passes. It reuses `workspace-overwrite` approval, source-digest revalidation, and atomic replacement. Approval and Action Audit show only the path, not edit text. A partial result means the candidate may already be installed with unknown durability and must not be retried automatically. This is not unified diff, regex patching, or fuzzy patching.
...
`read_file`, `glob`, literal `grep`, and `list_directory` are the four current model-visible read-only tools, and they share the same at-most-six sequential executions per user turn with every other tool. `glob` accepts workspace-relative `/`-separated patterns with component `*`, `?`, bracket classes, and whole-component `**`; it returns only stably sorted non-symlink regular-file paths, reads no contents, follows no links, and requires hidden components to be matched with an explicit leading dot. `grep(query, include)` uses the same portable selector for UTF-8 regular files, then performs case-sensitive literal line search and emits stable JSONL ordered by path and 1-based line; it does not support regex, indexing, or `.gitignore`. `list_directory` adds one-level directory and entry-type observation without replacing recursive glob or content search. Explicit truncation never proves an omitted path, entry, or match was absent.
...
The fixed model-visible order is `read_file, glob, grep, write_file, edit_file, run_command, mkdir, move_file, delete_file, delete_directory, list_directory, copy_file, read_file_lines, stat_path, list_tree, grep_regex, patch_file`; all 17 share at most six sequential calls per user turn. A seventh request receives only the structured limit result and does not enter permission, approval, execution, or Action Audit; another tool request leaves the candidate turn uncommitted. The four added read/navigation tools reuse `workspace-read`, `patch_file` reuses `workspace-overwrite`, and existing action classes remain unchanged.

Tool batches A/B/C are complete, while Foundation 5A remains deferred. The canonical system prompt is v15, the provider adapter contract remains v15, and the empty full-context identity is `ctx-v1-ea0e03265910b48b3cd97e3ace999507379a5e5cf168c6898390870266df051f`. ToolArguments v1, ActionIdentity v1, `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. Recursive copying/deletion, ignore-aware or indexed search, fuzzy/free-form patching, directory movement, non-empty deletion, recursive mkdir, shell source strings, interactive PTYs, network tools, automatic retry/fallback, parallel tools, multiple agents, and remote services remain unavailable. See [ADRs 0037](./docs/decisions/0037-batch-a-bounded-workspace-navigation.md), [0038](./docs/decisions/0038-batch-b-process-isolated-regex-grep.md), and [0039](./docs/decisions/0039-batch-c-structured-exact-multi-edit-patch.md) for the tool batches and [ADR 0040](./docs/decisions/0040-shared-six-call-tool-budget.md) for the six-call budget.
