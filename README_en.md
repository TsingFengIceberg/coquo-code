...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, a bounded sequential `read_file`/`glob`/literal `grep`/`write_file`/`edit_file`/`run_command`/`mkdir`/`move_file`/`delete_file` tool loop, provider-owned model limits, target-specific preflight, switch-time screening, provider-neutral Effective Context, manual and automatic compaction, and target-aware resume are implemented. Foundation 4A connects permission, approval, durable Action Audit, and controlled writes end to end; Foundation 4B adds unique exact edit; Foundation 4C adds controlled local command execution, Foundation 4D adds controlled single-directory creation, and Foundation 4E adds controlled no-overwrite regular-file movement, and Foundation 4F now adds controlled regular-file deletion. Shell source strings, interactive PTYs, OS sandboxing, regex/fuzzy patching, directory deletion, and directory movement remain unavailable.
...
The current model-visible surface has the fixed order `read_file`, `glob`, literal `grep`, complete-content `write_file`, unique exact `edit_file`, direct-argv `run_command`, single-directory `mkdir`, regular-file `move_file`, and regular-file `delete_file`; all nine share at most three sequential calls per user turn. Commands still require `danger-full-access`. Mkdir, move_file, and delete_file use `workspace-create`, `workspace-move`, and `workspace-delete` respectively and share PermissionGate, approval, and durable Action Audit with other side-effecting tools.

Foundation 4F is complete. The canonical system prompt is v10, the provider adapter contract is v11, and the empty full-context identity is `ctx-v1-42200fbe6c48a76d91ac0dde71e12be0e41674b1ad06c8b82bf82a541e3049e8`; ToolArguments v1, ActionIdentity v1, `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. Regex/index/ignore-aware search, fuzzy or multi-edit patching, directory movement, empty-directory removal, recursive mkdir/delete, shell source strings, interactive PTYs, network tools, streaming, automatic retry/fallback, parallel tools, multi-agent operation, and remote services remain unavailable. The recommended next independent direction is separately designed empty-directory removal while recursive deletion remains prohibited. See [ADR 0033](./docs/decisions/0033-foundation-4f-controlled-regular-file-deletion.md).
