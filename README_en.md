...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, workspace-root `AGENTS.md` instructions, both Eval layers, foreground durable multi-Stage Tasks, 22 ordinary bounded tools including Host-selected Brave or Tavily independent web search, four Stage-scoped coordination tools, one Task-admission tool, and three natural-language Task lifecycle tools are implemented. A user can advance a Task with direct language such as "accept", "the plan is good, continue", and "I confirm completion"; the model submits a structured request, and the Host mutates Task state and continues the foreground Driver only after the ordinary Session Turn commits durably and the exact pending state is revalidated. Every Stage remains an ordinary Turn, while file and search actions continue through PermissionGate, approval, and Action Audit. On Linux, `run_command` requires bubblewrap and seccomp isolation: the workspace is the only Host-persistent writable area and command network sockets are denied; independent search uses a separate Host network boundary. The ordinary Turn budget remains eight calls per response, 32 tool requests per user turn, and 24 provider invocations with a final text-only invocation.
...
| Approve web searches interactively in the REPL | `BRAVE_SEARCH_API_KEY=... uv run leonervis-code --permission-mode danger-full-access --approval ask` |
| Allow web searches automatically in one-shot mode | `BRAVE_SEARCH_API_KEY=... uv run leonervis-code --permission-mode danger-full-access --approval auto prompt "Search for the official Python 3.14 release notes and list the sources"` |
...
`web_search(query, max_results)` uses a Host-selected fixed Brave or Tavily Search API; it accepts no model-selected URL and does not read result pages. Exactly one valid key selects that backend automatically. With both keys, either set `LEONERVIS_WEB_SEARCH_BACKEND=brave|tavily` or use `/search use <source> [source...]` in the REPL. `/search status` and `/search sources` inspect availability and activation; `/search reset` restores startup-environment selection. Sources are ordered and the first is primary. Current execution is explicitly `primary-only`: additional active sources reserve the future fan-out and result-fusion contract and are not silently queried. Selection is process-local and does not modify the Session. Brave uses fixed GET, while Tavily uses fixed basic search with automatic parameters, generated answers, raw content, and images disabled, documented as one basic-search credit. Queries are limited to 512 characters/2 KiB and return 1 through 10 bounded JSONL results under Host response, output, and 15-second timeout limits. This `network-read` action proceeds only under `danger-full-access` with `ask/auto`. Ask displays the full query, count, actual primary backend, and quota disclosure; that backend is bound into exact ActionIdentity while routine views redact the query. Timeout or transport uncertainty returns `partial` and must not be retried automatically. Printable-ASCII credentials come only from the named environment variables and never enter Session or audit.
...
| `/search status`, `/search sources` | Inspect Brave/Tavily credential availability, ordered activation, and primary without displaying keys |
| `/search use <source> [source...]` | Set ordered active sources for this process; current execution uses only the first primary source |
| `/search reset` | Clear the REPL override and restore startup-environment source selection |
...
Provider batching, structured tool ledgers, streaming/Markdown terminal output, Session management and usage audit, read-only Git observation, the fail-closed command sandbox, Foundation 5A, both Eval layers, structured model Task coordination, informed admission, natural-language Task lifecycle, and Brave/Tavily independent web search are complete. When a compatible provider returns canonically bounded arguments that violate an ordinary tool schema, the Host now returns a matching error ToolResult and lets the model correct it within the same Turn; invalid JSON, unknown tools, globally oversized arguments, and invalid Task control calls still fail closed. The deterministic Host Eval remains `host-baseline-v2`. Current versions are canonical system prompt v30, provider adapter contract v32, ToolArguments v1, ActionIdentity v1, `turn_committed` schema v8, `turn_failed` schema v2, new `session_resumed` records at v2, Action Audit schema v1, new `context_compacted` records at v4, new `task_admission_resolved` records at v1, new `task_admission_origin` records at v1, new Task Stage records at v2, Task plan proposals at v3, Task completion proposals at v2, Task reflections at v2, Task blockers at v1, and Task acceptance, pause, and Task-checkpoint records at v1; current Effective Context representations remain `ctx-v5`/`ctx-v6`. Existing Session and Task transcripts remain compatible without rewriting; the current empty full-context identity without project instructions is `ctx-v5-468d2b764f1b20902080a07d4a00f027eb531ea5651cc90c74b681956bbc80b9`. Provider-native search, general `web_fetch`, MCP, background Tasks, scheduling, SubAgents, teams, worktree orchestration, parallel Stages/Actions, automatic reviewer spending, general automatic retry, resource quotas, Host sandbox bypass, real-model Eval leaderboards, and remote services remain unavailable. See [ADR 0102](./docs/decisions/0102-bounded-independent-web-search.md) for the independent-search boundary, [ADR 0099](./docs/decisions/0099-recoverable-provider-tool-argument-validation.md) for the recoverable argument boundary, and [ADR 0098](./docs/decisions/0098-natural-language-task-lifecycle-handoffs.md) for natural-language lifecycle.
