...
> **Current status:** named provider profiles, real/offline runtimes, resumable Sessions, a bounded sequential `read_file`/`glob`/literal `grep`/`write_file`/`edit_file`/`run_command`/`mkdir`/`move_file` tool loop, provider-owned model limits, target-specific preflight, switch-time screening, provider-neutral Effective Context, manual and automatic compaction, and target-aware resume are implemented. Foundation 4A connects permission, approval, durable Action Audit, and controlled writes end to end; Foundation 4B adds unique exact edit; Foundation 4C adds controlled local command execution, Foundation 4D adds controlled single-directory creation, and Foundation 4E now adds controlled no-overwrite regular-file movement. Shell source strings, interactive PTYs, OS sandboxing, regex/fuzzy patching, delete, and directory movement remain unavailable.
...
The current model-visible surface has the fixed order `read_file`, `glob`, literal `grep`, complete-content `write_file`, unique exact `edit_file`, direct-argv `run_command`, single-directory `mkdir`, and regular-file `move_file`; all eight share at most three sequential calls per user turn. Commands still require `danger-full-access`. Mkdir is `workspace-create`; move_file is `workspace-move`, requires existing source/destination parents, rejects symlinks, stale state, cross-filesystem moves, and existing destinations, and shares PermissionGate, approval, and durable Action Audit with other side-effecting tools.

Foundation 4E is complete. The canonical system prompt is v9 and the provider adapter contract is v10; ToolArguments v1, ActionIdentity v1, `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. Regex/index/ignore-aware search, fuzzy or multi-edit patching, delete, directory movement, recursive mkdir, shell source strings, interactive PTYs, network tools, streaming, automatic retry/fallback, parallel tools, multi-agent operation, and remote services remain unavailable. The recommended next independent direction is file-only delete, followed separately by empty-directory removal, without introducing recursive deletion. See [ADR 0032](./docs/decisions/0032-foundation-4e-controlled-no-overwrite-file-move.md).
