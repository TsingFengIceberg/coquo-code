...
Use `prompt` for scripts and one-shot tasks; use the bare command for a stateful multi-turn REPL. Successful turns are saved automatically, and tool execution shows redacted `[tool 1/6] ...` status lines.
...
A Session is workspace-bound and stores successful turns in append-only JSONL. Use the `session` and `--resume` commands above to inspect, audit, and restore it; see [Implemented Foundations and Design Evolution](./docs/implemented-foundations_en.md) for complete replay, screening, and durability semantics.
...
Ctrl-D, EOF, or Ctrl-C while waiting for input exits normally. Terminal colors are enabled only on a TTY; set `NO_COLOR=1` to disable them. See [Implemented Foundations and Design Evolution](./docs/implemented-foundations_en.md) for complete context, compaction, resume, tool-budget, and read-tool semantics.
...
Tool batches A/B/C, the six-call budget, and redacted live tool activity are complete, while Foundation 5A remains deferred. The canonical system prompt is v15, the provider adapter contract remains v15, and the empty full-context identity is `ctx-v1-ea0e03265910b48b3cd97e3ace999507379a5e5cf168c6898390870266df051f`. ToolArguments v1, ActionIdentity v1, `turn_committed` schema v2, Action Audit schema v1, `context_compacted` v2/v3 replay, and `ctx-v1`/`ctx-v2` representations remain unchanged. Recursive copying/deletion, ignore-aware or indexed search, fuzzy/free-form patching, directory movement, non-empty deletion, recursive mkdir, shell source strings, interactive PTYs, network tools, automatic retry/fallback, parallel tools, multiple agents, and remote services remain unavailable. See [ADRs 0037](./docs/decisions/0037-batch-a-bounded-workspace-navigation.md), [0038](./docs/decisions/0038-batch-b-process-isolated-regex-grep.md), and [0039](./docs/decisions/0039-batch-c-structured-exact-multi-edit-patch.md) for the tool batches, [ADR 0040](./docs/decisions/0040-shared-six-call-tool-budget.md) for the six-call budget, and [ADR 0041](./docs/decisions/0041-live-redacted-tool-activity-events.md) for live activity.
