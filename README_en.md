...
Provider batching, structured tool ledgers, streaming/Markdown terminal output, Session management and usage audit, read-only Git observation, the fail-closed command sandbox, Foundation 5A, both Eval layers, and foreground durable Tasks are complete. Current versions are canonical system prompt v25, provider adapter contract v26, ToolArguments v1, ActionIdentity v1, `turn_committed` schema v8, `turn_failed` schema v2, new `session_resumed` records at v2, Action Audit schema v1, new `context_compacted` records at v4, new Task Stage records at v2, new Task acceptance contract/check records at v1, and current `ctx-v5`/`ctx-v6` representations. Legacy `session_resumed` v1, header-only Tasks, Task Stage v1, and historical context identities/checkpoints remain compatible; the current empty full-context identity without project instructions is `ctx-v5-7fefaa42ca4226a17e7312fc723ecb3add2b6e8c96a0ac02671e69048156d401`. Background Tasks, scheduling, SubAgents, teams, worktree orchestration, parallel Stages/Actions, general automatic retry, network tools, resource quotas, Host sandbox bypass, real-model Eval leaderboards, and remote services remain unavailable. See [ADR 0088](./docs/decisions/0088-foreground-task-stage-execution-and-recovery.md) for execution and recovery, [ADR 0089](./docs/decisions/0089-task-planning-acceptance-budgets-and-management.md) for planning and basic acceptance, [ADR 0090](./docs/decisions/0090-structured-task-acceptance-and-independent-review.md) for structured acceptance and independent review, and [ADR 0091](./docs/decisions/0091-resume-runtime-binding-at-the-durable-commit-point.md) for first-action binding after resume.
