...
`child prepare` freezes a bounded redacted read-only execution envelope and creates a detached Child Session without changing `latest`; `child run` then executes one foreground read-only Turn through the shared Agent runtime. `child start` and REPL `/child start <id>` append a ready Child to a workspace-bound durable queue and attempt to launch a restartable local worker process; the queue keeps at most 32 pending items, one worker uses at most four Child execution threads, and an idle worker exits after a bounded interval rather than becoming an unlimited daemon. `child status`, `/child status`, and `child worker` expose worker identity, heartbeat, queue state, active submissions, and orphan diagnostics. `child cancel` durably records a request before signaling a cooperative token; `child wait` observes durable Child state without relying on the submitting process, and `child recover` requeues a claimed `ready` Child only after a fresh execution lease proves it is unowned, while claimed `running`/`cancelling` work can only become `interrupted` after recovery ownership and is never automatically retried. `child handoff` publishes a bounded untrusted result tied to exact terminal evidence; `child deliver` commits a content-free receipt in the parent Session before Host rendering and never injects Child output into parent history or Effective Context. Legacy v1 leases remain fail-closed; this is not exactly-once execution or a distributed worker fleet.
...
- [Bounded Team Scheduler and Recovery](./docs/decisions/0142-bounded-team-scheduler-and-recovery.md): durable schedule waves, assignment provenance, one-Team leases, bounded Child reuse, cancellation, and Provider-free recovery.
- [Parent-Owned Team Control Approval and Reply Evidence](./docs/decisions/0143-model-visible-team-controls.md): eleven parent-model Team controls, separate approval/audit, ToolSet isolation, version migration, and reply/review evidence.
...
- [DeepSeek Harness study submodule](./learning-submodules/deepseek-harness/README.md): read-only comparison material, not a runtime dependency.
